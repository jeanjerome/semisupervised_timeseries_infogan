...
num_category = 4  # total categorical factor
num_cont = 1  # total continuous factor
...
with tf.sg_context(name='generator', size=(4, 1), stride=(2, 1), act='relu', bn=True):
    gen = (z.sg_dense(dim=1024)
           ...
           .sg_upconv(dim=64)
           .sg_upconv(dim=32)
           .sg_upconv(dim=1, act='sigmoid', bn=False).sg_squeeze())
...
# classified image
run_generator(np.arange(num_category).repeat(batch_size/num_category),
              np.ones(batch_size) * 0.5, np.ones(batch_size) * 0.5)
              #np.random.uniform(0, 1, batch_size), np.random.uniform(0, 1, batch_size))
...
for i in range(num_category):
    run_generator(np.ones(batch_size) * i,
                  np.linspace(0, 1, num_category).repeat(batch_size/num_category),
                  np.expand_dims(np.linspace(0, 1, num_category), axis=1).repeat(batch_size/num_category, axis=1).T.flatten(),
